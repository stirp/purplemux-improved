...
모든 정보가 `~/.purplemux/workspaces.json`에 저장됩니다. 워크스페이스는 purplemux-improved가 저장하고 복원하는 단위입니다. 브라우저를 닫아도 워크스페이스가 사라지지 않고, tmux가 쉘을 살려두며 레이아웃도 그대로 유지됩니다.
...
모든 변경은 즉시 `~/.purplemux/workspaces.json`에 반영됩니다. 직접 열어 백업할 수도 있습니다 — 전체 파일 구조는 [데이터 디렉토리](/purplemux-improved/ko/docs/data-directory/) 참고. 서버 실행 중에 이 파일을 지우면 purplemux-improved는 빈 워크스페이스로 폴백해 처음부터 다시 시작합니다.

## 다음으로

- **[탭 & 창](/purplemux-improved/ko/docs/tabs-panes/)** — 워크스페이스 안에서 분할, 순서 변경, 포커스 이동
- **[레이아웃 저장 & 복원](/purplemux-improved/ko/docs/save-restore/)** — 브라우저 종료와 서버 재부팅에서 워크스페이스가 살아남는 방식
- **[키보드 단축키](/purplemux-improved/ko/docs/keyboard-shortcuts/)** — 전체 바인딩 테이블
