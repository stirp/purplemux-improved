...
purplemux-improved — это веб-приложение, поэтому опыт зависит от браузера, в котором вы его открываете. Ниже — версии, на которых мы активно тестируем; более старые могут работать, но официально не поддерживаются.
...
purplemux-improved опирается на несколько современных браузерных API. Если какого-то нет, приложение мягко деградирует, но теряет соответствующую функцию.
...
| Clipboard API | Копирование `npx purplemux-improved@latest`, кода | Кнопка скрывается, если недоступно. |
...
В purplemux-improved встроена самопроверка в **Настройки → Проверка браузера**. Она прогоняет те же проверки, что перечислены выше, и показывает зелёный / янтарный / красный значок для каждой возможности — без необходимости читать спецификации.
...
Если у вас необычная конфигурация и что-то не работает, пожалуйста, [откройте issue](https://github.com/stirp/purplemux-improved/issues) с user agent и результатом самопроверки.
