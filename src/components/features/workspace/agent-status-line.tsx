...
interface IAgentStatusLineProps {
  tabId: string;
...
const AgentStatusLine = ({ tabId, enabled, provider }: IAgentStatusLineProps) => {
  ...
  const url = enabled && workspaceId && sessionId
    ? `/api/${provider}/status-line?${new URLSearchParams({ workspaceId, tabId, sessionId })}` : null;
  ...
  return (
    <div data-agent-status-line={provider} className="mx-auto w-full max-w-content shrink-0 border-t px-3 py-1.5 font-mono text-[11px] text-muted-foreground">
      <p className="whitespace-pre-wrap break-words">{visibleText}</p>
    </div>
  );
};

export default AgentStatusLine;
