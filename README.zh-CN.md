...
| 改进模块 | 主要功能 |
| --- | --- |
| **Git Worktree 子任务** | 右键工作区创建独立分支和 Worktree，或进入「管理 Worktrees」查看仓库下的全部工作树、实时状态，接入已有目录并安全清理。不同需求可在各自目录启动 Agent，并行开发。 |
...
「管理 Worktrees」在查看时每 10 秒刷新，支持发现终端创建的 Worktree。删除前会检查改动、未跟踪及被忽略的文件、关联的 Purplemux 会话和工作树状态；默认保留分支。可另行勾选删除已合入指定目标分支的本地分支。主工作树、锁定或状态未知的目录不可直接删除。上游进度使用本地引用，不会自动 fetch。
